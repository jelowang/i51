...
float __atan__(float a)
{
	...
	{
		iLog("denominator:%X0X",denominator);
	}
...
float _atan_(float arg)
{
   ...
   if(arg > sq2p1)
   {
		if( arg<0.0001&&arg>-0.0001) iLog("_atan_:arg:0X%X",arg);
	   return __MM_PIO2__ - __atan__(1 / arg);
   }

	if( arg+1<0.0001&&arg+1>-0.0001) iLog("_atan_:arg+1:0X%X",arg+1);
...
float mmAsin(float arg)
{
	...
	if(arg>0.7)
	{
		temp = __MM_PIO2__ - mmAtan(temp/arg);
	}
	else
	{
		if( temp<0.0001&&temp>-0.0001) iLog("mmAsin:temp:0X%X",temp);
		temp = mmAtan(arg/temp);
	}
...
{
	if(__fsin__(mmFloatToReal(theta) + MM_PIO2)>-0.0001&&__fsin__(mmFloatToReal(theta) + MM_PIO2)<0.0001)
	{
		iLog("denominator:%X0X",__fsin__(mmFloatToReal(theta) + MM_PIO2));
	}
...
/*inline*/ float mmAtan2(float arg1, float arg2)
{
	...
	if( arg2<0.0001&&arg2>-0.0001) iLog("mmAtan2:arg2:0X%X",arg2);

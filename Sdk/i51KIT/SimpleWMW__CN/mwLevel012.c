...
iBOOL mwInitLevel012( void )
{
	...
	for( i=0; i<3; i++){
		for( j=0; j<10; j++){
#if defined(PKIG_SCREEN_240X320)
			iVectInit2(&pos,(j)*6*COORDINATE_RATE+95*COORDINATE_RATE,(i)*6*COORDINATE_RATE+100*COORDINATE_RATE);

#elif defined(PKIG_SCREEN_240X400)	
			iVectInit2(&pos,(j)*6*COORDINATE_RATE+95*COORDINATE_RATE,(i)*6*COORDINATE_RATE+130*COORDINATE_RATE);

#elif defined(PKIG_SCREEN_320X480)	
			iVectInit2(&pos,(j)*6*COORDINATE_RATE+130*COORDINATE_RATE,(i)*6*COORDINATE_RATE+146*COORDINATE_RATE);
	...
	mwLevelDrawBKwall();
	//i51KitG2DrawImage(mwContainer, MudLevel012, iNULL, iNULL);
	//i51KitG2DrawImage(mwContainer, RockLevel012, iNULL, iNULL);

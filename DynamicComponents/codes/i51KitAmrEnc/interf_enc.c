      ...
      TXFrameType frame_type, enum Mode speech_mode )
{
   ...
   i51AdeStdMemset16(stream, 0, block_size[mode]);
      ...
      int force_speech )
{
   ...
   if (noHoming){
      ...
      i51AdeStdMemset16(&prm[homing_size], 0, (PRMNO_MR122 - homing_size) << 1);
      used_mode = mode;
   }
...
/*
* get memory size used in Encoder
* return memory size
*/
int Encoder_Interface_GetMemorySize()
{
	return ( sizeof( enc_interface_State ) + Speech_Encode_Frame_GetMemorySize() );
}
 ...
 * Parameters:
 *    dtx               I: DTX flag
 *	  memory			memory Needed in Encoder
 * Function:
 ...
 */
void * Encoder_Interface_init( int dtx, char *memory, int memory_size)
{
	enc_interface_State *s = NULL;
	char	*DataBuf = memory;

	if ( NULL == memory || memory_size < Encoder_Interface_GetMemorySize() )
	{
		return NULL;
	}

	s = (enc_interface_State *)DataBuf;
	DataBuf += sizeof(enc_interface_State);

	s->encoderState = Speech_Encode_Frame_init( dtx, DataBuf );

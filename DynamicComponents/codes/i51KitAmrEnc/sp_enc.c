...
#define ACCURACY	0.00001 //精度

#define i51_abs(x) ((x)>0 ? (x) : (-x))

double i51_cos(double x);
double i51_acos(double x);
float  i51_fabs(float x);
float  i51_sqrt(float x);
double i51_floor(double x);
double i51_frexp( double x, int *expptr);
double i51_log10(double x); 
...
{
	int	   n = 0, Value=0;
	double DoublePI = PI + PI;
	double SquareX = 0, Result = 0, NextResult = 1;
 	...
 	SquareX = x * x;
  	...
  	while ( (NextResult > ACCURACY || NextResult < -ACCURACY) && n < 10)
  	{
  		Result += ( 0 == n%2 ) ? NextResult : -NextResult;
  		n++;
  		Value += 2;
  		NextResult = NextResult * SquareX /(double)(Value*(Value-1));
  	}
...
{
	int	   n = 0;
	double SquareX = x * x, Result = PI/2, NextResult = x;
	double Value = 0;
 	...
 	if (x >= 1)
 	{
  	...
  	while ( (NextResult > ACCURACY || NextResult < -ACCURACY) &&  n < 10)
  	{
  		Result -= NextResult;
  		Value = 2*n;
  		Value = ((Value+1)*(Value+1)*(Value+2)) / ( 4*(Value+3)*(n+1)*(n+1) ) ;
  		NextResult = NextResult * SquareX * Value;
  		n++;
  	}
...
double i51_floor(double x)
{
	return (double)( (x <= -ACCURACY) ? (int)(x-1) : (int)x );
}
...
double i51_frexp( double x, int *expptr )
{
	...
	//得到移码，并减去1022得到指数值。
	*expptr = (int)(((u.c[7] & 0x7f) << 4) | (u.c[6] >> 4)) - 1022;
...
{
	int		index = 0, i = 0;
	double	exp = 0, dValue = 0, Trasnation=-1;
	double	prama[5] = {0.7516, 1.2128, 0.6266, 0.1893, 0.0238};
	...
	for (i=0; i<5; i++)
	{
		dValue +=  prama[i] * Trasnation;
		Trasnation = -Trasnation * exp;
	}

	return ( (double)index/3.321928 + dValue);
...
static void Autocorr( Float32 x[], Float32 r[], const Float32 wind[] )
{
    ...
    */
   i51AdeStdMemset16( (void*)&y[L_WINDOW], 0, 44 );
...
static void Levinson( Float32 *old_A, Float32 *r, Float32 *A, Float32 *rc )
{
   ...
   i51AdeStdMemcpy16( rc, rct, 4 * sizeof( Float32 ) );
   i51AdeStdMemcpy16( old_A, A, MP1 * sizeof( Float32 ) );
}
...
static void Az_lsp( Float32 a[], Float32 lsp[], Float32 old_lsp[] )
{
   ...
   if ( nf < M ) {
      i51AdeStdMemcpy16( lsp, old_lsp, M <<2 );
   }
   return;
}
...
static void Lsp_lsf( Float32 lsp[], Float32 lsf[] )
{
   ...
   for ( i = 0; i < M; i++ ) {
      lsf[i] = ( Float32 )( i51_acos( lsp[i] )*SCALE_LSP_FREQ );
   }
   return;
}
...
static void Lsf_lsp( Float32 lsf[], Float32 lsp[] )
{
   ...
   for ( i = 0; i < M; i++ ) {
      lsp[i] = ( Float32 )i51_cos( SCALE_FREQ_LSP * lsf[i] );
   }
   return;
}
      ...
      *pred_init_i = 0;
      ...
      for ( j = 0; j < PAST_RQ_INIT_SIZE; j++ ) {
         ...
         if ( pred_init_err < min_pred_init_err ) {
            ...
            i51AdeStdMemcpy16( lsf_r1, temp_r1, M <<2 );
            i51AdeStdMemcpy16( lsf_p, temp_p, M <<2 );
            i51AdeStdMemcpy16( past_rq, (void*)&past_rq_init[j * M], M <<2 );
            *pred_init_i = j;
         }
      }
   }
      ...
      lsp_new[], Word16 **anap )
{
   ...
   i51AdeStdMemcpy16( lsp_old, lsp_new, M <<2 );
   i51AdeStdMemcpy16( lsp_old_q, lsp_new_q, M <<2 );
}
      ...
      Float32 error[] )
{
    ...
    */
   i51AdeStdMemcpy16( ai_zero, Ap1, MP1 <<2 );
   ...
   i51AdeStdMemcpy16( exc, res2, L_SUBFR <<2 );
      ...
      Word32 t_max, Float32 corr_norm[] )
{
   ...
   for ( i = t_min; i <= t_max; i++ ) {
      ...
      if ( norm == 0 )
         ...
         norm = ( Float32 )( 1.0 / ( i51_sqrt( norm ) ) );
...
static void Pred_lt_3or6( Float32 exc[], Word32 T0, Word32 frac, Word16 flag3 )
{
   ...
   for ( j = 0; j < L_SUBFR; j += 4 ) {
      ...
      exc[j] = ( Float32 )i51_floor(s + 0.5F);
      x1 = x0++;
      x2 = x0;
      s = x1[0] * c1[0] + x2[0] * c2[0];
      s += x1[ - 1] *c1[6] + x2[1] * c2[6];
      ...
      s += x1[ - 8] * c1[48] + x2[8] * c2[48];
      s += x1[ - 9] * c1[54] + x2[9] * c2[54];
      exc[j + 1] = ( Float32 )i51_floor(s + 0.5F);
      x1 = x0++;
      x2 = x0;
      s = x1[0] * c1[0] + x2[0] * c2[0];
      s += x1[ - 1] *c1[6] + x2[1] * c2[6];
      ...
      s += x1[ - 8] * c1[48] + x2[8] * c2[48];
      s += x1[ - 9] * c1[54] + x2[9] * c2[54];
      exc[j + 2] = ( Float32 )i51_floor(s + 0.5F);
      x1 = x0++;
      x2 = x0;
      s = x1[0] * c1[0] + x2[0] * c2[0];
      s += x1[ - 1] *c1[6] + x2[1] * c2[6];
      ...
      s += x1[ - 8] * c1[48] + x2[8] * c2[48];
      s += x1[ - 9] * c1[54] + x2[9] * c2[54];
      exc[j + 3] = ( Float32 )i51_floor(s + 0.5F);
   }
   return;
}
      ...
      Float32 gain_cand[], Word32 gain_cind[] )
{
   ...
   err_min = ( Float32 )i51_fabs( *gain - qua_gain_pitch[0] );
   index = 0;

   for ( i = 1; i < NB_QUA_PITCH; i++ ) {
      if ( qua_gain_pitch[i] <= gp_limit ) {
         err = ( Float32 )i51_fabs( *gain - qua_gain_pitch[i] );
      ...
      pos_max[], Word16 nb_track, Word32 ipos[], Word16 step )
{
   ...
   k_cn = ( Float32 )( 1 / i51_sqrt( sum ) );
   sum = 0.01F;
   sum += (Float32)Dotproduct40( dn, dn );
   k_dn = ( Float32 )( 1 / i51_sqrt( sum ) );
      ...
      cod[], Float32 h[], Float32 y[], Word32 sign_indx[], Word32 pos_indx[] )
{
   ...
   i51AdeStdMemset16( cod, 0, L_CODE <<2 );
      ...
      Word32 *gcode0_exp, Word32 *gcode0_fra, Float32 *en )
{
   ...
   if ( mode == MR122 ) {
      ...
      *gcode0_fra = ( ener >> 1 ) - ( *gcode0_exp << 15 );
      ...
      i51_frexp( ( Float64 )ener, &exp_code );
...
static Word16 q_gain_code( Float32 gcode0, Float32 *gain, Word32 *qua_ener_index)
{
   ...
   err_min = i51_fabs( *gain - ( gcode0 * *p++ ) );
   index = 0;

   for ( i = 1; i < NB_QUA_CODE; i++ ) {
      err = i51_fabs( *gain - ( gcode0 * *p++ ) );
   ...
   p = &gain_factor[index];
   *gain = (Float32)i51_floor(gcode0 * *p);
      ...
      Float32 gain_pit, Float32 en[], Float32 *ltpg )
{
   ...
   if ( en[3] > 0 && en[0] != 0 ) {
      /* gain = ResEn / LTPResEn */
      pred_gain = en[0] / en[3];
      *ltpg = ( Float32 )( i51_log10( pred_gain ) / i51_log10( 2 ) );
   }
   else {
      *ltpg = 0;
   }
      ...
      Word32 *qua_ener_index )
{
   ...
   coeff[0] = ( Float32 )( i51_sqrt( alpha * en[0] ) );
      ...
      Float32 *mem_w0, Float32 *exc, Float32 *sharp )
{
   ...
   *sharp = gain_pit;
   ...
   for ( i = 0; i < L_SUBFR; i += 4 ) {
      exc[i + i_subfr] = (Float32)i51_floor((gain_pit * exc[i + i_subfr] + gain_code * code[i]) + 0.5F);
      exc[i + i_subfr + 1] = (Float32)i51_floor((gain_pit * exc[i + i_subfr + 1] + gain_code * code[i
            + 1]) + 0.5F);
      exc[i + i_subfr + 2] = (Float32)i51_floor((gain_pit * exc[i + i_subfr + 2] + gain_code * code[
            i + 2]) + 0.5F);
      exc[i + i_subfr + 3] = (Float32)i51_floor((gain_pit * exc[i + i_subfr + 3] + gain_code * code[
            i + 3]) + 0.5F);
   }
      ...
      Float32 speech[], Float32 *log_en_hist )
{
   ...
   *hist_ptr += 1;
   ...
   i51AdeStdMemcpy16( &lsp_hist[ * hist_ptr * M], lsp_new, sizeof( Float32 )*M );
   ...
   if ( frame_en > 1 ) {
      log_en_hist[ * hist_ptr] = ( Float32 )( i51_log10( frame_en * 0.00625F )*
            1.660964F );
   }
   else {
      ...
      compute_sid_flag, Float32 past_rq[], Word32 *past_qua_en, Word16 **anap )
{
    ...
    */
      log_en = 0;
      i51AdeStdMemset16( lsp, 0, sizeof( Float32 )*M );
      ...
      *log_en_index = ( Word16 )( ( log_en * 4 ) + 0.5F );   /* 6 bits */
      ...
      if (*log_en_index > 46){
		k = NB_QUA_CODE+VQ_SIZE_HIGHRATES+VQ_SIZE_LOWRATES+(MR475_VQ_SIZE*2) + 46;
         past_qua_en[0] = k;
         past_qua_en[1] = k;
         past_qua_en[2] = k;
         past_qua_en[3] = k;
		 ...
		 k = NB_QUA_CODE+VQ_SIZE_HIGHRATES+VQ_SIZE_LOWRATES+(MR475_VQ_SIZE*2) + *log_en_index;
         past_qua_en[0] = k;
         past_qua_en[1] = k;
         past_qua_en[2] = k;
         past_qua_en[3] = k;
      ...
      count1, Word16 count2, Word16 ind_m, Word16 ind_a, Word16 scale )
{
   ...
   for ( i = count1; i < count2; i++ ) {
      temp1 += ( Float32 )i51_fabs( data[ind_m * i + ind_a] );
   }
   level = temp1 + *sub_level;
   *sub_level = temp1;

   for ( i = 0; i < count1; i++ ) {
      level += ( Float32 )i51_fabs( data[ind_m * i + ind_a] );
   }
   return( scale * level );
}
#endif
...
static void first_filter_stage( Float32 in[], Float32 out[], Float32 data[] )
{
   ...
   for ( i = 0; i < L_SUBFR; i++ ) {
	  j = 4*i;
      temp0 = ( in[j + 0] * 0.25F ) - ( COEFF5_1 * data0 );
      temp1 = data0 + ( COEFF5_1 * temp0 );
      temp3 = ( in[j+1]*0.25F )-( COEFF5_2 * data1 );
      temp2 = data1 + ( COEFF5_2 * temp3 );
      out[j + 0] = temp1 + temp2;
      out[j + 1] = temp1 - temp2;
      data0 = ( in[j + 2] * 0.25F ) - ( COEFF5_1 * temp0 );
      temp1 = temp0 + ( COEFF5_1 * data0 );
      data1 = ( in[j + 3] * 0.25F ) - ( COEFF5_2 * temp3 );
...
static void filter_bank( vadState *st, Float32 in[], Float32 level[] )
{
   ...
   for ( i = 0; i < FRAME_LEN / 4; i++ ) {
	   j=4 * i;
      filter5( &tmp_buf[j], &tmp_buf[j + 2], st->a_data5[1] );
      filter5( &tmp_buf[j +1], &tmp_buf[j + 3], st->a_data5[2] );
   }

   for ( i = 0; i < FRAME_LEN / 8; i++ ) {
	   j = 8 * i;
      filter3( &tmp_buf[j], &tmp_buf[j + 4], &st->a_data3[0] );
      filter3( &tmp_buf[j + 2], &tmp_buf[j + 6], &st->a_data3[1] );
      filter3( &tmp_buf[j + 3], &tmp_buf[j + 7], &st->a_data3[4] );
   }

   for ( i = 0; i < FRAME_LEN / 16; i++ ) {
	   j = 16 * i;
      filter3( &tmp_buf[j], &tmp_buf[j + 8], &st->a_data3[2] );
      filter3( &tmp_buf[j + 4], &tmp_buf[j + 12], &st->a_data3[3] );
   }

   /* calculate levels in each frequency band */
   /* 3000 - 4000 Hz*/
   level[8] = level_calculation( tmp_buf, &st->sub_level[8], 32, 40, 4, 1, 1 );

   /* 2500 - 3000 Hz*/
   level[7] = level_calculation( tmp_buf, &st->sub_level[7], 16, 20, 8, 7, 2 );

   /* 2000 - 2500 Hz*/
   level[6] = level_calculation( tmp_buf, &st->sub_level[6], 16, 20, 8, 3, 2 );

   /* 1500 - 2000 Hz*/
   level[5] = level_calculation( tmp_buf, &st->sub_level[5], 16, 20, 8, 2, 2 );

   /* 1000 - 1500 Hz*/
   level[4] = level_calculation( tmp_buf, &st->sub_level[4], 16, 20, 8, 6, 2 );

   /* 750 - 1000 Hz*/
   level[3] = level_calculation( tmp_buf, &st->sub_level[3], 8, 10, 16, 4, 2 );

   /* 500 - 750 Hz*/
   level[2] = level_calculation( tmp_buf, &st->sub_level[2], 8, 10, 16, 12, 2 );

   /* 250 - 500 Hz*/
   level[1] = level_calculation( tmp_buf, &st->sub_level[1], 8, 10, 16, 8, 2 );

   /* 0 - 250 Hz*/
   level[0] = level_calculation( tmp_buf, &st->sub_level[0], 8, 10, 16, 0, 2 );
}
#endif
...
static void vad_pitch_detection( vadState *st, Word32 T_op[] )
{
   ...
   for ( i = 0; i < 2; i++ ) {
      if ( i51_abs( st->oldlag - T_op[i] ) < LTHRESH ) {
         lagcount += 1;
      }
      ...
      Word16 ana[], enum Mode *used_mode, Float32 synth[] )
{
   ...
   i51AdeStdMemcpy16( st->new_speech, new_speech, L_FRAME <<2 );
   ...
   if ( *used_mode == MRDTX ) {
      ...
      i51AdeStdMemset16( st->old_exc, 0, ( PIT_MAX + L_INTERPOL )<<2 );
      i51AdeStdMemset16( st->mem_w0, 0, M <<2 );
      i51AdeStdMemset16( st->mem_err, 0, M <<2 );
      i51AdeStdMemset16( st->zero, 0, L_SUBFR <<2 );
      i51AdeStdMemset16( st->hvec, 0, L_SUBFR <<2 );
      i51AdeStdMemset16( st->lspSt->qSt->past_rq, 0, M <<2 );
      i51AdeStdMemcpy16( st->lspSt->lsp_old, lsp_new, M <<2 );
      i51AdeStdMemcpy16( st->lspSt->lsp_old_q, lsp_new, M <<2 );
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      if ( ( evenSubfr != 0 ) && ( *used_mode == MR475 ) ) {
         i51AdeStdMemcpy16( mem_syn_save, st->mem_syn, M <<2 );
         i51AdeStdMemcpy16( mem_w0_save, st->mem_w0, M <<2 );
         i51AdeStdMemcpy16( mem_err_save, st->mem_err, M <<2 );
         sharp_save = st->sharp;
      }
      ...
      /* MR475 */
         ...
         if ( evenSubfr != 0 ) {
            i51AdeStdMemcpy16( h1_sf0, st->h1, L_SUBFR <<2 );
         }
      }

      /* copy the LP residual (res2 is modified in the CL LTP search) */
      i51AdeStdMemcpy16( res2, res, L_SUBFR <<2 );
      ...
      if ( *used_mode != MR475 ) {
         ...
         if ( evenSubfr != 0 ) {
            i_subfr_sf0 = i_subfr;
            i51AdeStdMemcpy16( xn_sf0, xn, L_SUBFR <<2 );
            i51AdeStdMemcpy16( y2_sf0, y2, L_SUBFR <<2 );
            i51AdeStdMemcpy16( code_sf0, code, L_SUBFR <<2 );
            T0_sf0 = T0;
            T0_frac_sf0 = T0_frac;
             ...
             */
            i51AdeStdMemcpy16( st->mem_err, mem_err_save, M <<2 );
 ...
 *    succeed = 0
 */
static Word32 Pre_Process_init( Pre_ProcessState **state, char *memory)
{
   Pre_ProcessState *s = NULL;

   if ( state == ( Pre_ProcessState * * )NULL || NULL == memory ) 
   {
      return -1;
   }
      ...
      *x1, Word16 *speech, Float32 *f_speech )
{
   ...
   if ( ( i51_fabs( *y1 )+i51_fabs( *y2 ) ) < 0.0000000001 )
      *y2 = *y1 = 0;
}
...
static void cod_amr_reset( cod_amrState *s, Word32 dtx )
{
   ...
   /* reset Q_plsfState */
   i51AdeStdMemset16( s->lspSt->qSt->past_rq, 0, sizeof( Float32 )*M );
   i51AdeStdMemcpy16( s->lspSt->lsp_old, (void*)lsp_init_data, sizeof( lsp_init_data ) );
   i51AdeStdMemcpy16( s->lspSt->lsp_old_q, (void*)lsp_init_data, sizeof( lsp_init_data ) );
   ...
   i51AdeStdMemset16( s->gainQuantSt->adaptSt->ltpg_mem, 0, sizeof( Float32 )*LTPG_MEM_SIZE
         );
   s->gainQuantSt->sf0_gcode0_exp = 0;
   s->gainQuantSt->sf0_gcode0_fra = 0;
   s->gainQuantSt->sf0_target_en = 0.0F;
   i51AdeStdMemset16( s->gainQuantSt->sf0_coeff, 0, sizeof( Float32 )*5 );
   ...
   /* reset tonStabState */
   s->tonStabSt->count = 0;
   i51AdeStdMemset16( s->tonStabSt->gp, 0, sizeof( Float32 )*N_FRAME );

   /* reset LevinsonState */
   s->lpcSt->LevinsonSt->old_A[0] = 1.0F;
   i51AdeStdMemset16( &s->lpcSt->LevinsonSt->old_A[1], 0, sizeof( Float32 )*M );
   ...
   for ( i = 0; i < DTX_HIST_SIZE; i++ ) {
      i51AdeStdMemcpy16( &s->dtxEncSt->lsp_hist[i * M], (void*)lsp_init_data, sizeof( Float32 )*M);
   }
   i51AdeStdMemset16( s->dtxEncSt->log_en_hist, 0, M * sizeof( Float32 ) );
   ...
   i51AdeStdMemset16( s->old_speech, 0, sizeof( Float32 )*L_TOTAL );
   i51AdeStdMemset16( s->old_exc, 0, sizeof( Float32 )*( PIT_MAX + L_INTERPOL ) );
   i51AdeStdMemset16( s->old_wsp, 0, sizeof( Float32 )*PIT_MAX );
   i51AdeStdMemset16( s->mem_syn, 0, sizeof( Float32 )*M );
   i51AdeStdMemset16( s->mem_w, 0, sizeof( Float32 )*M );
   i51AdeStdMemset16( s->mem_w0, 0, sizeof( Float32 )*M );
   i51AdeStdMemset16( s->mem_err, 0, sizeof( Float32 )*M );
   i51AdeStdMemset16( s->ai_zero, 0, sizeof( Float32 )*L_SUBFR );
   i51AdeStdMemset16( s->hvec, 0, sizeof( Float32 )*L_SUBFR );
 ...
 *    succeed = 0
 */
static Word32 cod_amr_init( cod_amrState **state, Word32 dtx, char *memory)
{
	cod_amrState *s = NULL;
	char *data_buf = memory;

	if (  NULL == state || NULL == memory )
	{
		return -1;
	}

	s = ( cod_amrState * )data_buf;
	data_buf += sizeof( cod_amrState );

	/* init clLtpState */
	s->clLtpSt = ( clLtpState * )data_buf;
	data_buf += sizeof( clLtpState );

	/* init Pitch_frState */
	s->clLtpSt->pitchSt = ( Pitch_frState * )data_buf;
	data_buf += sizeof(Pitch_frState);

	/* init lspState */
	s->lspSt = ( lspState * )data_buf;
	data_buf += sizeof( lspState );

	/* init Q_plsfState */
	s->lspSt->qSt = ( Q_plsfState * )data_buf;
	data_buf +=  sizeof( Q_plsfState );

	/* init gainQuantState */
	s->gainQuantSt = ( gainQuantState * )data_buf;
	data_buf += sizeof( gainQuantState );

	/* init gc_predState x2 */
	s->gainQuantSt->gc_predSt = ( gc_predState * )data_buf;
	data_buf += sizeof(gc_predState);

	s->gainQuantSt->gc_predUncSt = ( gc_predState * )data_buf;
	data_buf += sizeof(gc_predState);

	/* init gain_adaptState */
	s->gainQuantSt->adaptSt = ( gain_adaptState * )data_buf;
	data_buf += sizeof( gain_adaptState );

	/* init pitchOLWghtState */
	s->pitchOLWghtSt = ( pitchOLWghtState * )data_buf;
	data_buf += sizeof(pitchOLWghtState);

	/* init tonStabState */
	s->tonStabSt = ( tonStabState * )data_buf;
	data_buf += sizeof(tonStabState);

	/* init lpcState */
	s->lpcSt = ( lpcState *)data_buf;
	data_buf += sizeof(lpcState);

	/* init LevinsonState */
	s->lpcSt->LevinsonSt = ( LevinsonState *)data_buf;
	data_buf += sizeof(LevinsonState);

	s->vadSt = ( vadState *) data_buf;
	data_buf += sizeof(vadState);

	/* Init dtx_encState */
	s->dtxEncSt = ( dtx_encState * )data_buf;
	data_buf += sizeof(dtx_encState);
...
/*
* get memory size used in sp_enc
* return memory size
*/
int	Speech_Encode_Frame_GetMemorySize(void)
{
	return ( sizeof(Speech_Encode_FrameState) + sizeof(Pre_ProcessState) +
		sizeof( cod_amrState ) + sizeof( clLtpState ) + 
		sizeof(Pitch_frState) + sizeof( lspState ) +
		sizeof( Q_plsfState ) + sizeof( gainQuantState ) +
		sizeof(gc_predState) + sizeof(gc_predState) +
		sizeof( gain_adaptState ) + sizeof(pitchOLWghtState) + 
		sizeof(tonStabState) + sizeof(lpcState) +
		sizeof(LevinsonState) + sizeof(vadState) + 
		sizeof(dtx_encState) );
}
 ...
 *    succeed = 0
 */
void * Speech_Encode_Frame_init( int dtx, char *memory)
{
	Speech_Encode_FrameState *s = NULL;
	char *data_buf = memory;
	...
	s = (Speech_Encode_FrameState *)data_buf;
	s->pre_state = NULL;
	s->cod_amr_state = NULL;
	s->dtx = dtx;

	data_buf += sizeof(Speech_Encode_FrameState);

	if ( Pre_Process_init( &s->pre_state, data_buf ) || 
		cod_amr_init( &s->cod_amr_state, dtx , (data_buf + sizeof(Pre_ProcessState)) ) )
	{
...
{
   if ( NULL == ( Speech_Encode_FrameState * )( *st ) )
   {
      return;
   }

...
static Word32 Syn_filt( Word32 a[], Word32 x[], Word32 y[], Word32 lg, Word32 mem[]
      , Word32 update )
{
   Word32 tmp[50];   /* i51AdeOsMalloc is slow */
      ...
      enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      /* i51AdeOsFree phase dispersion adaption */
...
static void Decoder_amr_exit( Decoder_amrState **state )
{
   if ( state == NULL || *state == NULL )
      return;
   i51AdeOsFree( ( *state )->lsfState );
   i51AdeOsFree( ( *state )->ec_gain_p_st );
   i51AdeOsFree( ( *state )->ec_gain_c_st );
   i51AdeOsFree( ( *state )->pred_state );
   i51AdeOsFree( ( *state )->background_state );
   i51AdeOsFree( ( *state )->ph_disp_st );
   i51AdeOsFree( ( *state )->Cb_gain_averState );
   i51AdeOsFree( ( *state )->lsp_avg_st );
   i51AdeOsFree( ( *state )->dtxDecoderState );
...
static int Post_Process_reset( Post_ProcessState *state )
{
   if ( ( Post_ProcessState * )state == NULL ) {
      i51AdeOsLog(0, "Post_Process_reset: invalid parameter\n" );
      return-1;
   }
   state->y2_hi = 0;
   state->y2_lo = 0;
...
static int Decoder_amr_init( Decoder_amrState **state )
{
   ...
   if ( ( Decoder_amrState * )state == NULL ) {
      i51AdeOsLog(0, "Decoder_amr_init: invalid parameter\n" );
      return-1;
   }
   *state = NULL;

   /* allocate memory */
   if ( ( s = ( Decoder_amrState * ) i51AdeOsMalloc( sizeof( Decoder_amrState ),__FILE__,__LINE__ ) ) ==
         NULL ) {
      i51AdeOsLog(0, "Decoder_amr_init: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* DPlsf_init */
   /* allocate memory */
   if ( ( s->lsfState = ( D_plsfState * ) i51AdeOsMalloc( sizeof( D_plsfState ),__FILE__,__LINE__ ) ) ==
         NULL ) {
      i51AdeOsLog(0, "DPlsf_init: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* ecGainPitchInit */
   /* allocate memory */
   if ( ( s->ec_gain_p_st = ( ec_gain_pitchState * ) i51AdeOsMalloc( sizeof(
         ec_gain_pitchState ) ,__FILE__,__LINE__) ) == NULL ) {
      i51AdeOsLog(0, "ecGainPitchInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* ecGainCodeInit */
   /* allocate memory */
   if ( ( s->ec_gain_c_st = ( ec_gain_codeState * ) i51AdeOsMalloc( sizeof(
         ec_gain_codeState ),__FILE__,__LINE__ ) ) == NULL ) {
      i51AdeOsLog(0, "ecGainCodeInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* gcPredInit */
   /* allocate memory */
   if ( ( s->pred_state = ( gc_predState * ) i51AdeOsMalloc( sizeof( gc_predState ) ,__FILE__,__LINE__) )
         == NULL ) {
      i51AdeOsLog(0, "gcPredInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* Cb_gain_averageInit */
   /* allocate memory */
   if ( ( s->Cb_gain_averState = ( Cb_gain_averageState * ) i51AdeOsMalloc( sizeof(
         Cb_gain_averageState ) ,__FILE__,__LINE__) ) == NULL ) {
      i51AdeOsLog(0, "Cb_gain_averageInit: can not i51AdeOsMalloc state structure\n" )
      ;
      return-1;
   }
   ...
   /* lsp_avgInit */
   /* allocate memory */
   if ( ( s->lsp_avg_st = ( lsp_avgState * ) i51AdeOsMalloc( sizeof( lsp_avgState ),__FILE__,__LINE__ ) )
         == NULL ) {
      i51AdeOsLog(0, "lsp_avgInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* Bgn_scdInit */
   /* allocate memory */
   if ( ( s->background_state = ( Bgn_scdState * ) i51AdeOsMalloc( sizeof( Bgn_scdState
         ) ,__FILE__,__LINE__) ) == NULL ) {
      i51AdeOsLog(0, "Bgn_scdInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* phDispInit */
   /* allocate memory */
   if ( ( s->ph_disp_st = ( ph_dispState * ) i51AdeOsMalloc( sizeof( ph_dispState ),__FILE__,__LINE__ ) )
         == NULL ) {
      i51AdeOsLog(0, "phDispInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }

   /* dtxDecInit */
   /* allocate memory */
   if ( ( s->dtxDecoderState = ( dtx_decState * ) i51AdeOsMalloc( sizeof( dtx_decState )
         ,__FILE__,__LINE__) ) == NULL ) {
      i51AdeOsLog(0, "dtxDecInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }
...
static int Post_Filter_reset( Post_FilterState *state )
{
   if ( ( Post_FilterState * )state == NULL ) {
      i51AdeOsLog(0, "Post_Filter_reset: invalid parameter\n" );
      return-1;
   }
...
static int Post_Filter_init( Post_FilterState **state )
{
   ...
   if ( ( Post_FilterState * )state == NULL ) {
      i51AdeOsLog(0, "F057:invalid parameter\n" );
      return-1;
   }
   *state = NULL;

   /* allocate memory */
   if ( ( s = ( Post_FilterState * ) i51AdeOsMalloc( sizeof( Post_FilterState ) ,__FILE__,__LINE__) ) ==
         NULL ) {
      i51AdeOsLog(0, "F057:can not i51AdeOsMalloc filter structure\n" );
      return-1;
   }
   s->agc_state = NULL;

   /* allocate memory */
   if ( ( s->agc_state = ( agcState * ) i51AdeOsMalloc( sizeof( agcState ) ,__FILE__,__LINE__) ) == NULL )
   {
      i51AdeOsLog(0, "agcInit: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }
   Post_Filter_reset( s );
   *state = s;
...
static int Post_Process_init( Post_ProcessState **state )
{
   ...
   if ( ( Post_ProcessState * )state == NULL ) {
      i51AdeOsLog(0, "Post_Process_init: invalid parameter\n" );
      return-1;
   }
   *state = NULL;

   /* allocate memory */
   if ( ( s = ( Post_ProcessState * ) i51AdeOsMalloc( sizeof( Post_ProcessState ),__FILE__,__LINE__ ) ) ==
         NULL ) {
      i51AdeOsLog(0, "Post_Process_init: can not i51AdeOsMalloc state structure\n" );
      return-1;
   }
 ...
 *    Void
 */
//void Speech_Decode_Frame_exit( void **st )//Nika
void Speech_Decode_Frame_exit( void *st )
{
   ...
   /* deallocate memory */
   i51AdeOsFree( (( Speech_Decode_FrameState * )st) );
   return;
}
 ...
 *    -1 = failure
 */
//int Speech_Decode_Frame_reset( void **st )//Nika
...
void * Speech_Decode_Frame_init( )
{
   ...
   /* allocate memory */
   if ( ( s = ( Speech_Decode_FrameState * ) i51AdeOsMalloc( sizeof(
         Speech_Decode_FrameState ) ,__FILE__,__LINE__) ) == NULL ) {
      i51AdeOsLog(0, "Speech_Decode_Frame_init: can not i51AdeOsMalloc state "
            "structure\n" );
      return NULL;
   }

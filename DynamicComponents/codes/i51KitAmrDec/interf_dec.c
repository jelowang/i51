...
void * Decoder_Interface_init( void )
{
   ...
   /* allocate memory */
   if ( ( s = ( dec_interface_State * ) i51AdeOsMalloc( sizeof( dec_interface_State ) ,__FILE__,__LINE__) ) ==
         iNULL ) {
      i51AdeOsLog(0, "Decoder_Interface_init: "
            "can not i51AdeOsMalloc state structure\n" );
      return iNULL;
   }
   s->decoder_State = Speech_Decode_Frame_init( );

   if ( s->decoder_State == iNULL ) {
      i51AdeOsFree( s );
      return iNULL;
   }
      ...
      Word16 *synth, int bfi)
{
   ...
   if ( resetFlag == 0 ) {
      Speech_Decode_Frame_reset( (pvoid *)(s->decoder_State) );
   }
